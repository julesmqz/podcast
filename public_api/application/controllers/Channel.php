...
class Channel extends REST_Controller {
	...
	public function rss_get( $key ){
		// exit( $id );
		$channel = $this->channel->getRss( $key );
		if( $channel != null ){
			$this->set_response($channel, REST_Controller::HTTP_OK);
		}
		else
	...
	public function rss_post( $id ){
		// exit( $id );
		$rss = $this->channel->createRss( $id );

		if( $rss != null ){
			$this->set_response($rss, REST_Controller::HTTP_OK);
		}
		else
		{
			$this->set_response(null, REST_Controller::HTTP_NOT_FOUND);
	...
	public function episodes_get( $channelId ){
		$episodes = $this->channel->getEpisodes( $channelId );
